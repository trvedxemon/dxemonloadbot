...
def ytdownloadvid(link, itag, audio):
    ...
    ytobject1 = ytobject.streams.get_by_itag(itag)
    ytobject2 = ytobject.streams.get_by_itag(audio)
    try:
        ytobject1.download(output_path=filepath)
        ytobject2.download(output_path=filepath)
        filename1 = filepath + ytobject1.default_filename
        filename2 = filepath + ytobject2.default_filename
        video = ffmpeg.input(filename1)
        audio = ffmpeg.input(filename2)
        out = merged_filepath + ytobject1.default_filename
